...
def handle_content_block_start(event: ContentBlockStartEvent) -> dict[str, Any]:
    ...
    if "toolUse" in start and start["toolUse"]:
        ...
        # Preserve model-specific data (e.g., Gemini thought signatures)
        if "metadata" in tool_use_data:
            current_tool_use["metadata"] = tool_use_data["metadata"]
...
def handle_content_block_stop(state: dict[str, Any]) -> dict[str, Any]:
    ...
    if current_tool_use:
        ...
        # Preserve model-specific data (e.g., Gemini thought signatures)
        if "metadata" in current_tool_use:
            tool_use["metadata"] = current_tool_use["metadata"]
